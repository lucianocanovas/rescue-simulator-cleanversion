            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RIGHT:
                    if self.map_manager.player1.vehicles or self.map_manager.player2.vehicles:
                        ...
                        # Pasamos el turno actual; next_turn puede usar ese número para eventos periódicos
                        self.map_manager.next_turn(self.current_turn)
                        # Incrementamos el contador de turno y guardamos el estado RESULTANTE
                        self.current_turn += 1
                        try:
                            saved_file = self.map_manager.save_game(self.current_turn)
                            print(f"Juego guardado en {saved_file}")
                        except Exception as e:
                            print(f"Error guardando turno {self.current_turn}: {e}")
